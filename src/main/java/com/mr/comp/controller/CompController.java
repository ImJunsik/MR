...
public class CompController  extends BaseController {
    ...
    public String compFileManage(HttpServletRequest req, Integer mrReqNo, Model model){
        ...
        //자료등록 체크 리스트 카운트 조회 yoo240626
        List<Integer> cntList = compService.selectCompCheckCount(mrReqNo);

        logger.info(cntList.get(0) + "] cnt : " + cntList.get(1));
        compFileVO.setUseCnt(cntList.get(0) * 100 + cntList.get(1));
        ...
        String mail = req.getParameter("c");
        if(mail != null)
        	if(mail.equals("c"))
        		compFileVO.setUseCnt(8282);
    ...
    public String compFileSave(CompFileManageVO compFileManageVO, CompFileVO compFileVO, Model model){
    	...
    	List<CompChrgrChgHistVO> compFileCountList = null;
        
        //자료등록관리 담당자 등록여부
        compFileCountList = compService.countCompFile(compFileManageVO);
        
        
        //compFileCount.setCheckCnt("0");		//yoo test용
        logger.info(compFileCountList.get(0).getCheckCnt() + " : <<<<getCheckCnt>>>> : " + compFileCountList.get(1).getCheckCnt());
        //자료등록관리 담당자 저장(수정), 자료등록관리 진행상세 저장(수정)
        if(compFileCountList.get(0).getCheckCnt().equals("0")) {
            compService.insertCompFile(compFileManageVO, compFileVO);
        } else {
            compService.updateCompFile(compFileManageVO, compFileVO);
        }
        
        if(compFileCountList.get(1).getCheckCnt().equals("0")) {
            compService.insertCompCheckList(compFileVO);
        } else {
            compService.updateCompCheckList(compFileManageVO);
        }
        char code = 'X';
        if(compFileManageVO.getpRtnCode() != null)
        	code = compFileManageVO.getpRtnCode().charAt(3);
        logger.info(" compFileManageVO.getpRtnCode() : " + compFileManageVO.getpRtnCode());
        if(!compFileManageVO.getpRtnCode().equals("Z02D"))			//yoo 240619 기술검토자일 때는 임시저장 시 메일을 보내지 않는다 (자료등록확인요청 시 보냄)
        	compService.requestDoc(compFileManageVO.getMrReqNo(),code, compFileManageVO.getpRtnMsg());
     ...
     * 자료등록확인 요청
     */
    @RequestMapping(value = "/requestDoc.do")
    public String requestDoc(CompFileManageVO compFileManageVO, CompFileVO compFileVO, Model model){
    	
    	//자료등록관리 MR 마스터 조회 
        //compFileVO = compService.selectCompFile(mrReqNo);
    	//List<CompChrgrChgHistVO> list = compService.selectCompLine(compFileManageVO.getMrReqNo());
    	//자료등록 체크 리스트 카운트 조회 yoo240626
        List<Integer> cntList = compService.selectCompCheckCount(compFileManageVO.getMrReqNo());

        logger.info(cntList.get(0) + "] cnt : " + cntList.get(1));
    	//Z02D가 Z02A ,Z02I에게 메일을 보낸다
        if(cntList.get(0) > 0)
        	compService.requestDoc(compFileManageVO.getMrReqNo(),'D',compFileManageVO.getpRtnMsg());
        else
        	return "redirect:compFileManage.do?mrReqNo=" + compFileManageVO.getMrReqNo()+ "&c=e";
        return "redirect:compFileManage.do?mrReqNo=" + compFileManageVO.getMrReqNo()+ "&c=c";
    }
    ...
    private void compRptSave(CompRptVO compRptVO)
    {
    	compService.insertCompCheck(compRptVO);
    ...
    public String compRptSave(CompRptVO compRptVO, Model model){
    	...
    	compRptSave(compRptVO);			//yoo 230111 save
