 ...
 */
var _contextPathURL = "http://192.168.79.179/OpenEDM_R8" ;
//var _contextPathURL = "http://127.0.0.1/OpenEDM_R8" ;
//var _contextPath = "/OpenEDM_R8" ;
var _contextPath = _contextPathURL ;
...
//	var openEDM_contextPath = "/OpenEDM_R8" ;
	if (document.OpenFtp == null) {
		...
		div.innerHTML = '<OBJECT ID="OpenFTPCom"'
				...
				+ 'CODEBASE="'
				+ _contextPath
				+ '/common/edms/OpenEDM_R8.exe#version=1,3,20,0" width="0" height="0" visible=0 name="OpenFtp">'
				+ '</OBJECT>';
...
_Openfile.prototype.downloadFile = function(fileDTO, local_pathfile) {
	...
	var param = "";
	if (fileDTO.get("file_id") != null && fileDTO.get("file_id").length > 0)
		param += "&fileDTO.file_id=" + escape(fileDTO.get("file_id"));
	if (fileDTO.get("f_category") != null
			&& fileDTO.get("f_category").length > 0)
		param += "&fileDTO.f_category=" + escape(fileDTO.get("f_category"));
	if (fileDTO.get("f_category_id") != null
			&& fileDTO.get("f_category_id").length > 0)
		param += "&fileDTO.f_category_id="
				+ escape(fileDTO.get("f_category_id"));
	if (fileDTO.get("kind"))
		param += "&kind=" + fileDTO.get("kind");
	var _this = this;
	new Ajax.Request(
			url,
			{
...
_Openfile.prototype.downloadFileByFC = function(f_category, f_category_id,
		local_pathfile, kind) {
	var fileDTO = new Hash();
	fileDTO.set("f_category", f_category);
	fileDTO.set("f_category_id", f_category_id);
...
*/
_Openfile.prototype.addFileForAttachment = function(filePath, userid, com_code, mr_code, mr_file_id, method) {
//	var openEDM_contextPath = "/OpenEDM_R8" ;
	this.isFtp();
	...
	_contextPath = 'http://edims.oilbank.co.kr';			//yoo
	
	var comfile = this.getComplexFileName(filePath);
	var url = _contextPath + "/edms/fi/other/inUploadForAttachment.action";
	var param = new Hash();
	param.set("fileDTO.org_file", this.parsingFileName(filePath));
	param.set("fileDTO.com_org_file", this.parsingFileName(comfile));
	param.set("fileDTO.i_emp_id", userid) ;
	param.set("fileDTO.com_code", com_code) ;
	param.set("fileDTO.mr_code", mr_code) ;
	param.set("fileDTO.mr_file_id", mr_file_id) ;
...
//	var _contextPath = "/OpenEDM_R8" ;
	...
	var param = new Hash();
	param.set("workFlowFormDTO.doc_seq", doc_seqList) ;
	param.set("workFlowFormDTO.workflowDTO.wf_kind", "RMR") ;
	param.set("workFlowFormDTO.workflowAppDTO.remark", remark) ;
	param.set("workFlowFormDTO.appList.app_user", requser_id) ;
	param.set("workFlowFormDTO.documentDTO.mr_code", mr_code) ;
	param.set("workFlowFormDTO.workflowDTO.requser_id", requser_id) ;
	param.set("workFlowFormDTO.workflowAppDTO.app_user", requser_id) ;
	param.set("workFlowFormDTO.workflowDTO.emp_id", receiverid) ;
