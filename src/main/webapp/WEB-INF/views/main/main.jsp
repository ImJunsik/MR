...
<table border="1" cellspacing="0" style="display:none">
    ...
    <tr>
        ...
        <td class="center cursor tempSummaryTd"><c:out value="${tempSummary.col01}"/></td>
        <td class="center cursor tempSummaryTd"><c:out value="${tempSummary.col02}"/></td>
    </tr>
</table>
...
$(document).ready(function() {
	...
	$.drawTable=function(isTotal, procStCd, col0, col01, col02, col03, col04, col05, col06, col07, col08, col09, col10){
        var row=0;
        switch (procStCd) {
        case "Z0110":
            row=0;
		...
		/*
        for (var i=1; i< 10;i++){
        	console.log(eval("col0"+i))
        	console.log(parseInt($("#summary").children("tbody").children("tr:eq("+row+")").children("td").eq(i).html())+parseInt(eval("col0"+i)))
            if(row==1)	// not total
            {
                $("#summary").children("tbody").children("tr:eq("+row+")").children("td").eq(i).html(parseInt($("#summary").children("tbody").children("tr:eq("+row+")").children("td").eq(i).html())+parseInt(eval("col0"+i)));
            } else 	// total
            {
                $("#summary").children("tbody").children("tr:eq("+row+")").children("td").eq(i).html(eval("col0"+i));
            }
        }
		*/
     	// 2025-09-10 ijs 수정 col0부터 시작하도록 수정 (i=0부터)
        for (var i=0; i< 10;i++){
            var colValue = eval("col0" + (i === 0 ? "" : i));
            
            if(!isTotal) {  // total이 아닐 때는 누적
                var currentValue = parseInt($("#summary").children("tbody").children("tr:eq("+row+")").children("td").eq(i).html()) || 0;
                $("#summary").children("tbody").children("tr:eq("+row+")").children("td").eq(i).html(currentValue + parseInt(colValue));
            } else {  // total일 때는 덮어쓰기
                $("#summary").children("tbody").children("tr:eq("+row+")").children("td").eq(i).html(colValue);
            }
        }
    };
//not total
    "<c:forEach var='summary' items='${summary}'>";
    $.drawTable(false,"${summary.procStCd}","${summary.col01}","${summary.col02}","${summary.col03}","${summary.col04}","${summary.col05}","${summary.col06}","${summary.col07}","${summary.col08}","${summary.col09}","${summary.col10}");
    "</c:forEach>";
//total
    "<c:forEach var='appSummary' items='${appSummary}'>";
    $.drawTable(true, "${appSummary.procStCd}","${appSummary.col01}","${appSummary.col02}","${appSummary.col03}","${appSummary.col04}","${appSummary.col05}","${appSummary.col06}","${appSummary.col07}","${appSummary.col08}","${appSummary.col09}","${appSummary.col10}");
    "</c:forEach>";
