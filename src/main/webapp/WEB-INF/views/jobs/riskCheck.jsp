...
<div id="content_header">
    <h4 class="content_header title">위험성검토/PORC(2차 변경관리위원회)</h4>
</div>
<!-- button -->

<!-- HSW 재전송 추가  -->
<c:forEach var='appLine' items='${riskCheck.appLine}' varStatus="loop">
<script>
//alert("${modeClass}");
...
<input type="hidden" name="itemCd" value="RISK"/>
    ...
    <table class="table_row" border="1" cellspacing="0">
        ...
        <tr>
            ...
            <td>
               <div class="form_group">
               <c:if test="${modeClass!='IAD'}"><span class="agreeDate"></span></c:if>
               <c:if test="${modeClass!='IAD'}"><span class="agreeEndDate"></span></c:if>
...
<table class="table_row" border="1" cellspacing="0">
    ...
    <tr>
        ...
        <th scope="row"><a class="tooltips cursor">PORC <img src="images/icon_exam.png"/><span>- PORC필요 : 투자비 10억원 초과 <br>또는 법적필요/행정기관 권고<br>- 내용 공유 : 5천만원~10억원미만</span></a></th>
        <td>
            <div class="form_group">
            <input type="radio" class="porcActYn" name="porcActYn" value="Y" <c:if test="${riskCheck.porcActYn=='Y'}">checked </c:if> <c:if test="${modeClass!='IAD'}">disabled="disabled"</c:if>> PORC필요:1등급
            <input type="radio" class="porcActYn" name="porcActYn" value="S" <c:if test="${riskCheck.porcActYn=='S'}">checked </c:if> <c:if test="${modeClass!='IAD'}">disabled="disabled"</c:if>> 내용공유:2등급
            <input type="radio" class="porcActYn" name="porcActYn" value="N" <c:if test="${riskCheck.porcActYn=='N'}">checked </c:if> <c:if test="${modeClass!='IAD'}">disabled="disabled"</c:if>> 불필요:2등급
            <c:if test="${riskCheck.porcActYn=='Y' || riskCheck.porcActYn=='S'}">
...
$(window).ready(function() {
    ...
    $(".checkList").click(function() {
         $.popupPopup({url:"popup/mrCheckList.do?&mrReqNo="+$("#mrReqNo").val(), width:"820", height:"700", targetName:$(this).attr("data-role"), scroll:"yes"});
    });

    $(".hazop").click(function() {
        $.popupPopup({url:"popup/mrHazopStudy.do?&mrReqNo="+$("#mrReqNo").val(), width:"820", height:"700", targetName:$(this).attr("data-role"), scroll:"yes"});
    });
    ...
    $(".app").click(function () {
        ...
        if(confirm("승인 하시겠습니까?")) {
            $.removeComma();
            $('#pageLoadingWrap').show();
            $("form").attr("action", "mrRiskCheckApp.do").submit();            
        }
    });
    ...
    $(".return").click(function () {
        if(confirm("반려 하시겠습니까?")) {
            $.removeComma();
            $('#pageLoadingWrap').show();
            $("form").attr("action", "mrRiskCheckReturn.do").submit();
        }
    });
    	  ...
    	  case 'Y':		// 필요
    	  {
    		  console.log( 'Y' );
    		  	...
    		  	if(nInvstCose >= 1000000000)
	   	    	{
	   	    		$.popup({url:"popup/mrJobsPorc.do?mrReqNo="+$("#mrReqNo").val() + "&porcYn=" + selectValue, scroll:"yes", width:"880", height:"500"});
    		  ...
    		  if(nInvstCose < 50000000)
	  	    	{
    			    ...
    			    var retVal = confirm("해당 MR건이 투자비 5천만원 이상, 10억원 미만의 범위를 벗어 났습니다. 해당 건은 5천 미만 입니다. 계속 하시겠습니까?");
	  	     	    ...
	  	     	    if( retVal == true ){
	  	     		    ...
	  	     		    console.log("확인선택!");
	  	     		 $.popup({url:"popup/mrShareContent.do?mrReqNo="+$("#mrReqNo").val() + "&porcYn=" + selectValue, scroll:"yes", width:"880", height:"500"});
...
if("${riskCheck.porcActYn}" == "Y" && "${porcClass}" == "IASA") {
	$.popup({url:"popup/mrJobsPorcWrite.do?&mrReqNo="+$("#mrReqNo").val(), scroll:"yes", width:"880", height:"500"});
};
