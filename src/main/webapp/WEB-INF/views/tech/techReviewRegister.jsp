...
<%@ include file="/WEB-INF/includes/taglibs.jsp" %>
    ...
    <div class="button_top">
        ...
        <input type="image" src="images/btn_requestJobReview1.png" style="display:none" class="vam cursor appReq BAA"/>
	...
	<div id="content_wrap">
		<form id="registerForm" method="post">
			...
			<%--     <tr>
			    ...
			    <!-- 
			    <tr class="chkDate1">
			        <th class="dep2" scope="row">검사관점</th>
			        <td>
			              <div class="form_group">
			              <c:if test="${modeClass!='BAA' and modeClass!='BBA'}"><span class="checkFocusText"></span></c:if>
			              <c:if test="${modeClass=='BAA' or modeClass=='BBA'}"><input type="text" class="i_input checkFocusText" readOnly/></c:if>
			              <input type="hidden" class="checkFocusValue" name="jobs[11].chrgEmpNo" />
			              <input type="hidden" name="jobs[11].chrgrClCd" value="Z02H12"/>
			              <input type="hidden" class="checkFocusDutyText" name="jobs[11].thdayPos" />
			              <input type="hidden" class="checkFocusTeamText" name="jobs[11].thdayTeam" />
			              <input type="hidden" class="checkFocusTeamValue" name="jobs[11].chrgTeamNo" />
			              <img src="images/icon_search.png" class="vam cursor jobEmpSearch BAA BBA" data-role="checkFocus" style="display:none"/>
			              <c:if test="${modeClass!='BAA' and modeClass!='BBA'}"><span class="checkFocusSuggText"></span></c:if>
			              <c:if test="${modeClass=='BAA' or modeClass=='BBA'}"><input type="text" class="i_input_200 checkFocusSugg" name="jobs[11].sugg" placeholder="직무 요청사항 기술"/></c:if>
			              </div>
			        </td>
			    </tr> -->
			    </tbody>
			...
			<table class="table_row" border="1" cellspacing="0">
			    ...
			    <tr>
			        <th rowspan="6" scope="row">기술검토항목</th>
			    ...
			    <tr>
			        <th class="dep2" scope="row">온실가스 배출<span style="color:red">&nbsp;*</span></th>
			        <td colspan="3">
			            <div class="form_group TECH06_group">
			            <input type="hidden" name="rvRsts[5].itemCd" value="TECH06" />
			            <input type="radio" name="rvRsts[5].clCd01" class="i_radio BAA TECH06" value="1" disabled="disabled"/>있음
			            <input type="radio" name="rvRsts[5].clCd01" class="i_radio BAA TECH06" value="2" disabled="disabled"/>없음
			            <!-- span class="ml30">의견 : <c:if test="${modeClass!='BAA'}"><span class="TECH06_SUGG_text"></span></c:if><input type="text" name="rvRsts[5].totRvSugg" class="i_input_200 BAA TECH06_SUGG" value="" style="display:none"/></span> -->
			            </div>
			        </td>
			    </tr>
			    <tr>
			    ...
			    <tr>
			        <th rowspan="3" scope="row">경제성 검토</th>
			        <th rowspan="2" class="dep2" scope="row">정량적 효과</th>
			        <td>
			            <div class="form_group">
			            <c:if test="${modeClass=='BAA'}">
		    	...
		    	<div class="form_group">
			    	<c:if test="${modeClass=='BAA'}">
			        	<td colspan="3">온실가스 배출량 : <input type="text" class="i_input num number" name="zoutqty" id="zoutqty"  value="<fmt:formatNumber value="${techReview.zoutqty}" groupingUsed="true"/>" disabled>&nbsp;tCO2&nbsp;/&nbsp;탄소배출권 비용 : <input type="text" class="i_input num number" id="zoutamt" name="zoutamt" value="<fmt:formatNumber value="${techReview.zoutamt}" groupingUsed="true"/>" disabled>원</td>
			        </c:if>
			        <c:if test="${modeClass!='BAA'}">
			        	<td colspan="3">온실가스 배출량 : <fmt:formatNumber value="${techReview.zoutqty}" groupingUsed="true"/>&nbsp;tCO2&nbsp;/&nbsp;탄소배출권 비용 : <fmt:formatNumber value="${techReview.zoutamt}" groupingUsed="true"/>원</td>
			        </c:if>
			        </div>
			    </tr>
...
$(window).ready(function() {
	...
	if("${rvRst.itemCd}" !="") {
        $(".${rvRst.itemCd}").each(function () {
        	if($(this).val()=="${rvRst.clCd01}") {
        	    ...
        	    if("${rvRst.itemCd}" == 'TECH06') {
        	    	if("${rvRst.clCd01}" == '1')
        	        {
        	        	console.log('활성화 시킨다.');	
        	        	$('#zoutqty').removeAttr("disabled");
        	            $('#zoutamt').removeAttr("disabled");
        	        }else{
        	        	console.log('비활성화 시킨다.');
        	        	$('#zoutqty').attr("disabled", "disabled");
        	            $('#zoutamt').attr("disabled", "disabled");
        	        }
        	    }
	...
	$("input[name='rvRsts[5].clCd01']:radio").change(function () {
        //라디오 버튼 값을 가져온다.
    ...
    $.validation = function(){
        ...
        if(isValid && "${modeClass}"=="BAA") {
        	isValid = $.validator({
                ...
                TECH06 : {method:"class",type:"check",focus :"TECH06_group",msg:"온실가스 배출을 선택하세요."},
