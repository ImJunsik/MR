...
<div class="button_top">
	<c:forEach var='appLine' items='${register.appLine}' varStatus="loop">
		<script>
			var test5 = "${modeClass}";
			console.log(test5);
			console.log("${appLine.chrgrClCd}");
			console.log("${appLine.ifStatCd}");
			console.log("${appLine.chrgEmpNo}");
			console.log("${appLine.loginEmpNo}");
			console.log("======================")
		</script>
		...
		<c:if test="${appLine.chrgrClCd == 'Z02D'}">
			<c:if
				...
				<input type="image" src="images/btn_reRequestApproval.png" style="display: none" class="cursor reAppReq IBA" /><!-- 재상신 -->
			</c:if>
		</c:if>
	</c:forEach>
	<input type="image" src="images/btn_temp.png" class="save IAA" name="save" style="display: none" />
	<!-- input type="image" src="images/btn_completeCapex.png" class="capex IAA" name="capex" style="display:none" /--> <!-- 주석 처리 -->
	<input type="image" src="images/btn_requestApproval.png" class="appreq IAA" name="appreq" style="display: none" /> <!-- 승인요청 -->
	...
	<input type="image" src="images/btn_return.png" class="return " name="return" style="display:none" /><!-- 반려 -->
	<!-- <input type="image" src="images/btn_approve.png" class="appr " name="appr" style="display:none" /> -->
	
	
	<!--승인버튼 강제 활성화-->	
	<!-- <input type="image" src="images/btn_approve.png" name="appr" class="appr " /> -->	
	...
	<form id="registerForm" method="post">
		...
		<table class="table_row" border="1" cellspacing="0">
            ...
            <td>
                ...
                <c:choose>
				<c:when test="${modeClass eq 'IAA'}"><input type="text" class="i_input agreeText" readOnly/></c:when>
				<c:otherwise><span class="agreeText"></span></c:otherwise>
				</c:choose>
                ...
                <c:if test="${modeClass=='IAB'}"><img src="images/icon_search.png" class="cursor empSearch IAB" data-role="agree" style="display:none"/></c:if>
                <c:if test="${modeClass=='IAA'}"><img src="images/icon_search.png" class="cursor empSearch IAA" data-role="agree" style="display:none"/></c:if>
                </div>
            </td>
            <th scope="row">승인일</th>
            <td>
               <div class="form_group">
               <c:if test="${modeClass!='IAA'}"><span class="agreeDate"></span></c:if>
               <c:if test="${modeClass!='IAA'}"><span class="agreeEndDate"></span></c:if>
               <c:if test="${modeClass=='IAA'}">
                    승인기한 : <input type="text" name="appLine[1].procTrmDt" class="i_input inputDate limitDate agreeEndDate" value="" readOnly/>
               </c:if>
               <c:if test="${modeClass!='IAA'}">
...
console.log("gianServer : " + gianServer);
			...
			function() {
				...
				$(".appreq").click(
						function() {
							...
							//}
							var Z02F = document.getElementsByName('appLine[1].chrgEmpNo');
							if(Z02F != '')
								if ($.validation())
	...
	$(document).on(
			...
			function() {
				...
				/*var url = "http://" + gianServer + "gian.oilbank.co.kr/connlogin.aspx?" + "&uid="*/
				var url = "https://" + gianServer + "hdep.oilbank.co.kr/approval/legacy/goFormLink.do?" 
						+ "&uid="
