...
<div class="button_top">
    ...
    <!-- <input type="image" src="images/btn_approve.png" style="display:none" class="cursor app ABA ADA ACA"/> -->
...
$(window).ready(function() {
    ...
    ***************************************/
    ...
    if("${mrAction}" == "mrRqRegisterAppReq"){
        ...
        //var url = "https://hdep.oilbank.co.kr/approval/legacy/goFormLink.do?" //운영
        var url = "https://" + gianServer + "hdep.oilbank.co.kr/approval/legacy/goFormLink.do?" //개발/운영 자동 처리(gianServer)
...
$(document).on("click",".unitSearch", function () {
	...
	//yoo 240404 파라메터를 Key와 Value로 쪼개어 해쉬에 담는다.
    ...
    for(var i = 0; i < hashes.length; i++)
    {
        ...
        //yoo 240404 필요 항목만 파라메터러로 사용 한다
        if(hash[0].indexOf('procs') > -1 && hash[0].indexOf('.mrProcNo') > 0)
        {
        	vars.push(hash[0]);
        	vars[hash[0]] = hash[1];
        }
	...
	for (var j = 0; j < vars.length; j++) {
		console.log(vars[j] + ' - ' + vars[vars[j]]);
		//debugger;
		params += vars[j] + '=' + vars[vars[j]] + '&';
   	} 
	//params = params.substring(0,params.length -1);
	console.log('2) ' + params);
	/*
	var n = params.indexOf('chrgTeamNo=&mrReqTitle=&');
	
	if(n == -1)
		n = params.indexOf('&mrReqTitle');
	
	params = params.substring(n,params.length);
	*/
    //$.popup({url:"popup/unitSearch.do?"+ params +"&plantNo="+$(".plantNo").val() +"&plantName="+$(".plantNo option:selected").text(), width:"880", height:"500"});
    $.popup({url:"popup/unitSearch.do?"+ params +"plantNo="+$(".plantNo").val() +"&plantName="+$(".plantNo option:selected").text(), width:"880", height:"500"});
    console.log('3) ' + "popup/unitSearch.do?"+ params +"plantNo="+$(".plantNo").val() +"&plantName="+$(".plantNo option:selected").text());
    $("#selectUnit").find("tr").remove().end();
});
...
$(document).on("click",".equipSearch", function () {
    ...
    for(var i = 0; i < hashes.length; i++)
    {
      ...
      //yoo 240404 필요 항목만 파라메터러로 사용 한다
        if(hash[0].indexOf('equips') > -1 && hash[0].indexOf('.mr') > 0)
        {
        	vars.push(hash[0]);
        	vars[hash[0]] = hash[1];
        }
	...
	*/
	//params = params.substring(n,params.length);
	//params = params.substring(0,params.length -1);
    //$.popup({url:"popup/equipSearch.do?"+ params, width:"880", height:"600"});
    $.popup({url:"popup/equipSearch.do?"+ params +"plantNo="+$(".plantNo").val() +"&plantName="+$(".plantNo option:selected").text(), width:"880", height:"600"});
   
    console.log('3) ' + 'popup/equipSearch.do?'+ params +"plantNo="+$(".plantNo").val() +"&plantName="+$(".plantNo option:selected").text());
    $("#selectEquip").find("tr").remove().end();
});
