...
function setNssoConfiguration(baseUri, functionLogonFail, functionLogonSuccess, functionReceiveTfa, functionReceiveDuplication, functionSsoUnavailable) {
...
function parseFromSSO(json) {
    ...
    if (json.result) {
        ...
        if (json.errorCode === 0) {
            ...
            // login domains
            if (json.hasOwnProperty("moveUrl"))
                callUrls(json);
...
function webToEssoLogon(pathESSO, artifactID) {
    $.ajax({
        type: 'GET',
        url: pathESSO + "LogonArtifact",
        data: {
            time: new Date().getTime(),
...
function callUrls(json) {
    var moveUrls = json['moveUrl'];
    for (var i = 0; i < moveUrls.length; i++) {
        var moveUrl = moveUrls[i];
        if (moveUrl.urlType === "0")
            logonDomains(moveUrl, json.domainCred, (json.hasOwnProperty("dupinfo") ? json.dupinfo : ""), (json.hasOwnProperty("uccp") ? json.uccp : ""));
        else if (moveUrl.urlType === "1")
            clearNotAuthn(moveUrl);
    }
}

function clearNotAuthn(moveUrl) {
    $.ajax({
        type: 'POST',
        url: moveUrl.url,
        xhrFields: {withCredentials: true},
        ...
        error: function (request, status, error) {
            ssoErrorMessage = "Failed logon domain. " + error;
        }
    });
}

function logonDomains(moveUrl, domainCred, dupinfo, uccp) {
    $.ajax({
        type: 'POST',
        url: moveUrl.url,
        xhrFields: {withCredentials: true},
