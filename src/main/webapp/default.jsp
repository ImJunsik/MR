...
<%
    ...
    try
    {
        ...
        else if (status == AuthStatus.SSOFail)
        {
            ...
            else if (auth.getErrCode() == AgentExceptionCode.NotAuthenticatedApp.getValue())
            {
                // 인증 받지 않은 앱에서의 인증확인 요청
                logoutScript = "goLogout('" + errorMessage + "');";
            }
        }
...
<body>
<!-- 
사용자 계정:<%=WebUtil.stripTag(userId)%>
<hr/>
사용자 속성<br/>
<%=WebUtil.stripTag(userAttributes)%>
<hr/>
에러 코드:<%=WebUtil.stripTag(errorCode)%>
<hr/>
에러 메시지:<%=WebUtil.stripTag(errorMessage)%>
<hr/>
<a href="<%=WebUtil.stripTag(logoutUrl)%>">Logout</a>
<script type="text/javascript">
    <%=WebUtil.stripTag(logoutScript)%>
</script>
<hr/>
<a href="trust.jsp">trust</a>
-->
<form id="loginForm" method="post" action="/mr/ssoLogin.do">
  		<input type="hidden" class="int" tabindex="1" name="empNo" value="<%=WebUtil.stripTag(userId)%>"  />
 </form>  
